...
dframes=['one','two','three','four','five','six','seven','eight','nine','ten','eleven','twelve','thirteen',
...
for x, y in zip(years, dframes):
    values=dff[(dff['year']==x)&(dff['region']=='Europe')].nlargest(5, columns=['suicide rate (deaths per 100,000)']).sort_values(by=['year','suicide rate (deaths per 100,000)'])
    n_frame[y]=values
...
fig = go.Figure(
    data=[go.Bar(x=value['suicide rate (deaths per 100,000)'], y=value['country'],orientation='h',
                 text=value['suicide rate (deaths per 100,000)'], texttemplate='%{text:.3s}',
                 textfont={'size':18}, textposition='inside', insidetextanchor='middle',
                 width=0.9, marker={'color':value['color_code']})
        for key, value in n_frame.items() if key=='one'
    ],
    layout=go.Layout(
