...
for x, y in zip(years, dframes):
    values=df[(df['year']==x)&(df['region']=='Europe')].nlargest(5, columns=['suicide rate (deaths per 100,000)']\
           ).sort_values(by=['year','suicide rate (deaths per 100,000)'])
    n_frame[y]=values
...
#-------------------------------------------
fig = go.Figure(
    data=[go.Bar(x=n_frame['one']['suicide rate (deaths per 100,000)'], y=n_frame['one']['country'],orientation='h',
